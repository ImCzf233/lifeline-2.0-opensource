...
import Lifeline.wtf.events.world.EventPacketReceive;
import Lifeline.wtf.events.world.EventUpdate;
import Lifeline.wtf.gui.font.CFont;
import Lifeline.wtf.gui.font.CFontRenderer;
import Lifeline.wtf.gui.font.FontLoaders;
import Lifeline.wtf.management.EntityHelper;
...
import Lifeline.wtf.module.modules.player.Teams;
...
import Lifeline.wtf.utils.render.AnimationUtils;
...
public class KillAura
    ...
    private static final Numbers<Double> r = new Numbers<Double>("THUDRed", "THUDRed", 255.0, 0.0, 255.0, 2.0);
    private static final Numbers<Double> g = new Numbers<Double>("THUDGreen", "THUDGreen", 255.0, 0.0, 255.0, 2.0);
    private static final Numbers<Double> b = new Numbers<Double>("THUDBlue", "THUDBlue", 255.0, 0.0, 255.0, 2.0);
...
//    }
    ...
    public KillAura() {
        ...
        this.addValues(this.Mode, rotmode, thudMode, thudOption, this.Cps, Range, Blockreach, SwitchDelay, r,g,b, this.Esp, this.Autoblock, Players, Animals, Mobs, Invis);
    }
...
//        drawCircle(entity, entity.hurtTime >= 1 ? (new Color(255,0,0,160).getRGB()) : (new Color(HUD.r.getValue().intValue(),HUD.g.getValue().intValue(),HUD.b.getValue().intValue(),160).getRGB()),event);
    ...
    public void onRender2D(EventRender2D event) {
        ...
        if (thudMode.getValue() == THUDMode.Bad) {
            ...
            if (target != null && Esp.getValue()) {
                ...
                RenderUtil.drawRect(x - 1, y - 4.8, x + 100, y + height + 1, new Color(0, 0, 0, 100).getRGB());
                RenderUtil.drawHead((AbstractClientPlayer) target, x + 5, y + 4, 25, 25);
//                if (target.hurtTime == 0) {
//                    RenderUtil.drawRect(x, y , 500, 300, new Color(253, 1, 1, 50).getRGB());
//                }

                mc.fontRendererObj.drawStringWithShadow(target.getName(),x + 37, y + 7, new Color(255, 255, 255).getRGB());

                RenderUtil.drawRect(healthBarX, healhBarY, 604, healhBarY + healtBarHeight - 7, new Color(0,0,0,30).getRGB());
                //RenderUtil.drawRect(healthBarX, healhBarY, 604, healhBarY + healtBarHeight - 7, new Color(255, 0,0).getRGB());
                RenderUtil.drawRect(healthBarX, healhBarY, (float) healthBarX + var48 * var51 - 7.2, healhBarY + healtBarHeight - 5, new Color(255,255,255).getRGB());
                //RenderUtil.drawRoundedRect((float) healthBarX, (float) healhBarY, (float) ((float) healthBarX + var48 * var51 - 7.2), healhBarY + healtBarHeight - 5, 4, new Color(255,255,255).getRGB());
                fr.drawStringWithShadow(String.format("%.1f", Float.valueOf(var48)) + " HP", x + 50, healhBarY + 2, -1);
            }
        }


        if (thudMode.getValue() == THUDMode.Novo) {
            if (target != null && Esp.getValue()) {
                CFontRenderer fr = FontLoaders.kiona18;
                int height = 35;
                int healthBarX = x + 37;
                ...
                RenderUtil.drawRect(x - 1, y - 4.1, x + 100, y + height,new Color(37,35,39).getRGB());
                RenderUtil.drawHead((AbstractClientPlayer) target, x + 1, y - 2, 35, 35);
                mc.fontRendererObj.drawStringWithShadow(target.getName(),x + 39, y + 1, new Color(255, 255, 255).getRGB());
                RenderUtil.drawRect(healthBarX + 1, healhBarY - 5, 608, healhBarY + healtBarHeight - 9, new Color(25,25,25).getRGB());
                RenderUtil.drawRect(healthBarX + 1, healhBarY - 5, (float) healthBarX + var48 * var51 - 7.2, healhBarY + healtBarHeight - 9, new Color(r.getValue().intValue(), g.getValue().intValue(), b.getValue().intValue()).getRGB());
                mc.fontRendererObj.drawStringWithShadow(String.format("%.1f", Float.valueOf(var48)) + " %", x + 50, healhBarY - 3, -1);
            }
        }


        if (thudMode.getValue() == THUDMode.Romantic) {
            if (target != null && Esp.getValue()) {
                ...
                int healtBarHeight = 30;
                int healhBarY = y + height - healtBarHeight - 3;
                int healthPercent = (int) EntityHelper.getEntityHealthPercent((EntityPlayer) target);
                float var28 = target.getHealth() + target.getAbsorptionAmount();
                float var32 = target.getMaxHealth() + target.getAbsorptionAmount();
                float var37 = 35 + fr.getStringWidth("larissalarissa40");
                float var42 = (float) ((double) Math.round((double) var28 * 100.0) / 100.0);
                float var46 = 100.0f / var32;
                float var48 = var42 * var46;
                float var51 = (var37 - 50.0f) / 100.0f;
                RenderUtil.drawRect(x - 1, y - 3, x + 41 + fr.getStringWidth("larissalarissal"), y + height + 1, RenderUtil.transparency(new Color(37, 37, 37).getRGB(), 0.6f));
                RenderUtil.drawRect(x, y, x + 40 + fr.getStringWidth("larissalarissal"), y + height, RenderUtil.transparency(new Color(45, 45, 45, 255).getRGB(), 0.8f));
                RenderUtil.drawRect(x, y - 2, x + 40 + fr.getStringWidth("larissalarissal"), y, RenderUtil.transparency(new Color(HUD.r.getValue().intValue(), HUD.g.getValue().intValue(), HUD.b.getValue().intValue()).getRGB(), 0.8f));
                RenderUtil.drawHead((AbstractClientPlayer) target, x + 2, y + 2, 32, 32);
                try {
                    RenderUtil.drawRect(healthBarX, healhBarY, (float) healthBarX + var48 * var51 + 3.0f, healhBarY + healtBarHeight, RenderUtil.transparency(new Color(HUD.r.getValue().intValue(), HUD.g.getValue().intValue(), HUD.b.getValue().intValue()).getRGB(), 1f));
                    RenderUtil.drawRect(healthBarX, healhBarY, (float) healthBarX + var48 * var51 + 1.0f, healhBarY + healtBarHeight, RenderUtil.transparency(new Color(20, 20, 20).getRGB(), 0.5f));
                    fr.drawStringWithShadow(String.format("%.1f", Float.valueOf(var48)) + " HP", x + 51, healhBarY + 12.5, -1);
                } catch (Exception ex) {
                    ex.printStackTrace();
                }
            }
