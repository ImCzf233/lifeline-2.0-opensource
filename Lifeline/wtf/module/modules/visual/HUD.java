...
import Lifeline.wtf.utils.render.*;
...
import Lifeline.wtf.utils.math.RotationUtil;
...
public class HUD extends Module {
	...
	public Mode<Enum> trmode = new Mode<Enum>("TitleMode", "TitleMode", TRMode.values(), TRMode.Jello);
	...
	public static Option<Boolean> showfps = new Option<>("ShowFPS", "ShowFPS", true);
	...
	public HUD() {
		...
		this.addValues(this.mode, this.colormode, this.r, this.g, this.b, this.alpha,this.trmode,
				this.compassvalue, this.notification, this.crosshair, this.wingvalue, this.fpshurtcam,blackhotbar,
				this.onscreenradar, this.movescoreboardx, this.movescoreboardy, this.x, this.y, this.hidedebugline, this.rendertabui ,this.showfps);
	}
	...
	private void renderHud(EventRender2D event) {
		...
		CFontRenderer font = FontLoaders.J18;
		CFontRenderer font2 = FontLoaders.J18;
		CFontRenderer font3 = FontLoaders.J18;
		useFont = true;
...
//            else if (!(boolean)this.customlogo.getValue()) {
			...
			if (trmode.getValue().equals(TRMode.Big)) {
				FontLoaders.GBD40.drawStringWithShadow(Client.name, 2, 2, rainbow1.getRGB());
				FontLoaders.GBD202.drawStringWithShadow(Client.version, 72, 3, rainbow1.getRGB());
				FontLoaders.GBD202.drawStringWithShadow(mc.thePlayer.getName(), 72, 13, rainbow1.getRGB());
			}

			if (trmode.getValue().equals(TRMode.Topline)) {
				RenderUtil.drawRect(2, 2, 4 + font2.getStringWidth("  LIFELINE |  |" + aaaaa + mc.thePlayer.getName()), 3, new Color(r.getValue().intValue(), g.getValue().intValue(), b.getValue().intValue(), 255).getRGB());
				RenderUtil.drawRect(2, 3, 4 + font2.getStringWidth("  LIFELINE |  |" + aaaaa + mc.thePlayer.getName()), 15, new Color(30, 30, 30, 80).getRGB());
				font2.drawString(" LIFELINE | " + aaaaa + " | " + mc.thePlayer.getName(), 2 , 6, new Color(255, 255, 255).getRGB());
			}
			if (trmode.getValue().equals(TRMode.Jello)) {
				RenderUtil.drawImage1(new ResourceLocation("paimoncc/Lifeline.png"),10,10,126,50);
			}
			if (trmode.getValue().equals(TRMode.none)) {
				font2.drawStringWithShadow("L", 2, 2, rainbow1.getRGB());
				font2.drawStringWithShadow("ifeline", 8, 2, new Color(255, 255, 255).getRGB());
				font2.drawStringWithShadow("(", font2.getStringWidth("Lifeline") + 5, 2, new Color(101, 101, 101).getRGB());
				font2.drawStringWithShadow(aaaaa, font2.getStringWidth("Lifeline (") + 1.7, 2, new Color(255,255,255).getRGB());
				font2.drawStringWithShadow(")", font2.getStringWidth("Lifeline (" + aaaaa) + 2, 2, new Color(101, 101, 101).getRGB());
				...
				if (colormode.getValue() != ColorMode.Fade) {
					switch (mode.getModeAsString()) {
						case "Simple": {
							...
							RenderUtil.drawImage1(new ResourceLocation("paimoncc/arraylistshadow.png"), x - 8.0F - 2.0F - 1.0F, (float)(y + 2) - 2.5F - 1.5F - 1.5F - 1.5F - 9.0F - 1.0F, (float)(FontLoaders.J16.getStringWidth(this.getName()) + 44 + 25), (float)(FontLoaders.J16.getStringWidth(this.getName()) * 1 + 24 + 25));

							font.drawString(name, x - 3.0f, y + 2,
						...
						case "Side": {
							...
							font.drawString(name, x - 5.0f, y + 2,
									colormode.getValue() == ColorMode.Rainbow ? rainbow.getRGB()
											: new Color(r.getValue().intValue(), g.getValue().intValue(),
											b.getValue().intValue(), 255).getRGB());
					...
					switch (mode.getModeAsString()) {
						case "Simple": {
							...
							font.drawString(name, x - 3.0f, y + 2, color);
							break;
						}
						case "Rect": {
							//RenderUtil.drawImage1(new ResourceLocation("paimoncc/arraylistshadow.png"), x - 8.0F - 2.0F - 1.0F, (float)(y + 2) - 2.5F - 1.5F - 1.5F - 1.5F - 9.0F - 1.0F, (float)(FontLoaders.J16.getStringWidth(this.getName()) + 44 + 25), (float)(FontLoaders.J16.getStringWidth(this.getName()) * 1 + 24 + 25));
...
//	                    	Gui.drawRect(x - 9, y + 1, x - 7, y + 9, new Color(0,0,0,90).getRGB());
						...
						case "Side": {
							...
							font.drawString(name, x - 5.0f, y + 2, color);
							RenderUtil.drawBorderedRect(RenderUtil.width(), y - 1, RenderUtil.width() - 2, y + 11, 1.0f,
									color, color);
							break;
						}
			...
			String text3 = (((Object) EnumChatFormatting.WHITE) + "FPS:" + (Object) ((Object) EnumChatFormatting.GRAY) + Minecraft.getDebugFPS());
...
//                	if(!mc.gameSettings.keyBindPlayerList.isKeyDown()) {
			if (showfps.getValue() == true) {
				font3.drawStringWithShadow(text3, 2, RenderUtils.height() - n - 12, new Color(11, 12, 17).getRGB());
			}


			//font3.drawStringWithShadow(text3, 2, 476, new Color(11, 12, 17).getRGB());
			if (!mc.ingameGUI.getChatGUI().getChatOpen()) {
				font3.drawStringWithShadow(text2, 2, RenderUtils.height() - n, new Color(11, 12, 17).getRGB());
			}
//                	}
...
//               		}
	...
	private void drawPotionStatus(ScaledResolution sr) {
		CFontRenderer font = FontLoaders.Jello15;
