package Lifeline.wtf.utils.render;

import Lifeline.wtf.events.rendering.EventRender3D;
import Lifeline.wtf.module.modules.visual.HUD;
import Lifeline.wtf.module.tessellate.Tessellation;
import Lifeline.wtf.utils.ColorUtils2;
import Lifeline.wtf.utils.Helper;
import Lifeline.wtf.utils.math.Vec2f;
import Lifeline.wtf.utils.math.Vec3f;
import Lifeline.wtf.utils.particles.Particle;
import Lifeline.wtf.utils.render.gl.GLClientState;
...
public class RenderUtil {
	...
	public static void drawImage1(ResourceLocation image, int x, int y, int width, int height) {
		drawImage1(image, x, y, width, height, 0.4f);
	}

	public static void drawImage1(ResourceLocation image, float x, float y, float width, float height) {
		drawImage1(image, (int)x, (int)y, (int)width, (int)height, 0.4f);
	}
	public static void drawImage1(ResourceLocation image, int x, int y, int width, int height, float alpha) {
	...
	public static void drawCustomImage2(int x, int y, int width, int height, ResourceLocation image) {
		ScaledResolution scaledResolution = new ScaledResolution(Minecraft.getMinecraft());
	...
	public static void drawCustomImage2(float x, float y, int width, int height, ResourceLocation image) {
		ScaledResolution scaledResolution = new ScaledResolution(Minecraft.getMinecraft());
		GL11.glDisable((int) 2929);
		GL11.glEnable((int) 3042);
		GL11.glDepthMask((boolean) false);
...
//        GL11.glEnable(GL_MULTISAMPLE);
	...
	public static void drawRoundedRect2(float x, float y, float x2, float y2, final float round, final Color color) {
		x += (float) (round / 2.0f + 0.5);
		y += (float) (round / 2.0f + 0.5);
		x2 -= (float) (round / 2.0f + 0.5);
		y2 -= (float) (round / 2.0f + 0.5);
	...
	public static void drawRoundedRect2(float x, float y, float x2, float y2, final int round, final int color) {
		x += (float) (round / 2.0f + 0.5);
		y += (float) (round / 2.0f + 0.5);
		x2 -= (float) (round / 2.0f + 0.5);
		y2 -= (float) (round / 2.0f + 0.5);
...
//        GL11.glHint(3155, 4352);
	...
	public void roundedRect2(final double x, final double y, double width, double height, final double edgeRadius, final Color color) {
		final double halfRadius = edgeRadius / 2;
		width -= halfRadius;
		height -= halfRadius;
